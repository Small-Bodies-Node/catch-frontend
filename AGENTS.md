...
- Typescript
  ...
  - Prefer organizing shared utilities under `src/utils`.
...
- Functional Programming Paradigm
  ...
  - Generally prefer to move logic and data out of entry scripts and into dedicated files within `src/utils`.
  - Generally prefer functional programming patterns:
    ...
    - Prefer to place a single exported function inside a file of the same name placed within `/src/utils/`; if this file needs helper functions that are not used elsewhere, and if they are relatively short and simple, then place them in the same file.
