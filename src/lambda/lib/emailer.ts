...
export const emailer = async (
  ...
  if (!event.body) {
    callback(null, {
      ...failureResponse,
      body: JSON.stringify({ success: false, message: 'No body provided in request' })
    });
...
async function sendEmail(
  ...
  const isRecaptchaBypassed = process.env.RECAPTCHA_BYPASS_CODE === recaptchaBypassCode;
  const isRecaptchaRequired = !!process.env.RECAPTCHA_SECRET && !isRecaptchaBypassed;
