...
export class PanstarrsOverlayComponent implements OnInit, OnDestroy, AfterViewInit {
        ...
        .subscribe(([apiSelectedDatum, apiDataStatus]) => {
          ...
          this.pansstarrsApiService
            ...
            .subscribe((apiPayload) => {
              ...
              this.isAtlasOrNeat =
                imageUrl.toUpperCase().includes('NEAT') || imageUrl.toUpperCase().includes('ATLAS');
                ...
                const coordObservables = this.raDecs.map((raDecItem) =>
                  this.imageWcsService
                    ...
                    .pipe(
                      ...
                      catchError((error) => {
                        console.warn(
                          `WCS: Failed to get pixel coords for RA:${raDecItem.ra}, Dec:${
                            raDecItem.dec
                ...
                if (coordObservables.length > 0) {
                  this.wcsPixelCoordSubscription.add(
                    ...
                    forkJoin(coordObservables).subscribe({
                      next: (results) => {
                        this.pixelCoordinatesWCS = results.filter((r) => r !== null) as {
                          x: number;
                          y: number;
                          id: number;
                          ra: number;
  ...
  getDivWidth(raErr: number): number {
    ...
    const pixelWidth = (correctedRaErrAngular / angularFovWidth) * this.renderedImageWidth;
