...
export class TableDataComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
  //
  ...
  constructor(
    ...
    this.subscriptions.add(
      this.store$.select(selectApiDataShownColState).subscribe((colState) => {
        ...
        const y = x.filter(
          (key) =>
            this.colState![key as keyof TColStateData] && !this.nonHideableCols.includes(key),
        );

        this.shownHideableCols = y;
        this.allShownCols = [...this.nonHideableCols, ...this.shownHideableCols];
        this.rerenderTable();
      }),
    );
    this.subscriptions.add(
      this.store$.select(selectApiDataDownloadRowState).subscribe((downloadRowState) => {
  ...
  keyPress(event: KeyboardEvent) {
    ...
    let pageRowOfNewIndex = isArrowUp ? pageRowOfOldIndex - 1 : pageRowOfOldIndex + 1;

    if (pageRowOfNewIndex < 0) pageRowOfNewIndex = 0;
    if (pageRowOfNewIndex >= this.pageSize) pageRowOfNewIndex = this.pageSize - 1;
    if (pageRowOfNewIndex >= this.pageSize) pageRowOfNewIndex = this.pageSize - 1;

    let newSortedIndex = indexOfFirstRowOfPage + pageRowOfNewIndex;
    if (newSortedIndex >= sortedApiData.length) newSortedIndex = sortedApiData.length - 1;
  ...
  getSortedApiData(): IApiMovum[] | IApiFixum[] | undefined {
    ...
    if (this.dataSource && this.dataSource.filter) {
      data = data.filter((item) => this.dataSource!.filterPredicate(item, this.dataSource!.filter));
    }
  ...
  clickDownloadAllCheckbox() {
    ...
    this.store$.dispatch(ApiDataAction_SetDownloadRowState({ newDownloadRowState }));
  ...
  isRowSelected(element: IApiMovum) {
    const isRowSelected = element.product_id === this.apiSelectedDatum?.product_id;
