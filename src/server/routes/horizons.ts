...
/**
 *  Pings JPL horizons for ephemeris data; caches to AWS S3
 */
export const horizons = async (req: Request, res: Response) => {
  ...
  const COMMAND = body_type === 'comet' ? `DES=${target}%3B CAP%3B NOFRAG%3B` : `${target}%3B`;
  ...
  const cachedUrl = `${process.env['S3_BUCKET_BASE_URL']}/${filename}`;
